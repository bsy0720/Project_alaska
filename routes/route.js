...
router.get('/alaska_notice_write', (req, res) => {
    db.getAllNotice((rows) => {
        res.render('./sub/alaska_notice_write', {rows:rows});
    });
});
...
function(req, res, next) {
    ...
    if(errs['errors'].length > 0) {
        res.render('alaska_notice_write', {errs:errs['errors']});
    } else {
...
router.get('/NewNotice', (req,res) => {
    ...
    db.getNoticeById(id, (row) => {
        if(typeof id == 'undefined' || row.length <= 0) {
            ...
            res.render('./sub/alaska_notice_NewWrite', {row:row[0]});
        }
    });
});

//수정한거 보내기(공지사항)
router.post('/NewNotice', [check('content').isLength({min:1, max: 6000})], (req,res) => {
    let errs = validationResult(req);
    let param = JSON.parse(JSON.stringify(req.body));
    let id = param['id'];
    let writer = param['writer'];
    ...
    if (errs['errors'].length > 0) {
        db.updateNoticeById(id, (row) => {
            res.render('alaska_notice_NewWrite', {row:row[0], errs:errs['errors']})
        })
    } else {
...
router.get('/alaska_Q&A_write', (req, res) => {
    db.getAllQna((rows) => {
        res.render('./sub/alaska_Q&A_write', { rows:rows });
    });
})
...
[check('content').isLength({ min: 1, max: 3000})],
    function(req, res, next) {
        ...
        if (errs['errors'].length > 0) { //에러출력
            res.render('alaska_Q&A_write', {errs:errs['errors']});
        } else {
            let param = JSON.parse(JSON.stringify(req.body));
            let writer = param['writer'];
            let pass = param['pass'];       
...
router.get('/qna_page', (req, res) => {
    let id = req.query.id;
        db.getQnaById(id, (row) => {
            if(typeof id == 'undefined' || row.length <= 0) {
                res.status(404).json({error: 'undefined alaska'})
            } else {
    ...
    router.get('/newQna', (req,res) => {
        ...
        db.getQnaById(id, (row) => { // 이쪽은 id값과 일치하는 row만 불러오는 id 값을 가지고오기
            if(typeof id == 'undefined' || row.length <= 0) {
                ...
                res.render('./sub/alaska_Q&A_newWrite', {row:row[0]});
            }
        });
    });

    //수정한거 보내기(여행문의)
    router.post('/newQna', [check('content').isLength({min: 1, max: 3000})], (req,res) => {
        let errs = validationResult(req);
        let param = JSON.parse(JSON.stringify(req.body));
        let id = param['id'];
        let writer = param['writer'];
        ...
        if (errs['errors'].length > 0) {
            db.updateQnaById(id, (row) => {
                res.render('alaska_Q&A_newWrite', {row:row[0], errs:errs['errors']})
            })
        } else {
...
router.get('/trip_review_write', (req, res) => {
    db.getAllReview((rows) => {
        res.render('./sub/alaska_trip_review_write', {rows:rows});
    });   
});
...
[check('content').isLength({min: 1, max: 6000})],
    function(req, res, next) {
        ...
        console.log(errs);
        if(errs['errors'].length > 0) {
            res.render('alaska_trip_review_write', {errs:errs['errors']});
        } else {
            let param = JSON.parse(JSON.stringify(req.body));
            let writer = param['writer'];
            let pass = param['pass'];
    ...
    router.get('/trip_review_page', (req, res) => {
        ...
        db.getReivewById(id, (row) => {
            if(typeof id == 'undefined' || row.length <= 0) {
                ...
                res.render('./sub/alaska_trip_review_page',{row:row[0]});
            }
        })
    });
    ...
    router.get('/newReview', (req,res) => {
        ...
        db.getReivewById(id, (row) => {
            if(typeof id == 'undefined' || row.length <= 0) {
                ...
                res.render('./sub/alaska_trip_review_Newwrite',{row:row[0]});
            }
        });
    });

    //수정한거 보내기(여행후기)
    router.post('/newReview', [check('content').isLength({min: 1, max: 6000})], (req,res) => {
        let errs = validationResult(req);
        let param = JSON.parse(JSON.stringify(req.body));
        let id = param['id'];
        let writer = param['writer'];
        ...
        if (errs['errors'].length > 0) {
            db.updateReviewById(id, (row) => {
                res.render('alaska_trip_review_Newwrite', {row:row[0], errs:errs['errors']})
            })
        } else {
